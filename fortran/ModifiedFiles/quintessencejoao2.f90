    ...
    !Need to specify field_potential function, and also initial_phi
	...
	! I still have to eliminate TEarlyQuintessence completely so I should actually use the old names for variables and then refactor to my names!
    ...
    type, extends(TDarkEnergyModel) :: TQuintessence
        ...
        class(CAMBdata), pointer, private :: State ! used to refer to the rest of the system
    contains
    procedure :: field_potential !V(phi) potential [+ any cosmological constant]
    ...
    ! Specific implementation for my quintessence models.
    type, extends(TQuintessence) :: TQuintessenceJoao
	integer :: potential_type ! Allows me to change the potential functional form
	real(dl), dimension(3) :: potentialparams ! Parameters for the potential V(phi).  Can change the number of params if neede
    contains
    procedure :: field_potential
    procedure :: Init => TQuintessenceJoao_Init
    procedure :: ReadParams =>  field_getpotentialparams
    ...
    end type TQuintessenceJoao

    procedure(TClassDverk) :: dverk

    public TQuintessence, TQuintessenceJoao
    contains

    !function VofPhi(this, phi, deriv) 				! OLD FUNCTION VOFPHI, USING FIELD_POTENTIAL NOW
    ...
    function field_potential(quintessence, phi, potentialparams) result(VofPhi) ! This function returns an array with V(phi), V'(phi) and V''(phi), necessary quantities to the equations. Manage units!!
	! Refactor to consider array result!!

        !The input variable phi is sqrt(8*Pi*G)*psi
        !Returns (8*Pi*G)^(1-deriv/2)*d^{deriv}V(psi)/d^{deriv}psi evaluated at psi
        !return result is in 1/Mpc^2 units [so times (Mpc/c)^2 to get units in 1/Mpc^2]

	class(TQuintessenceJoao), intent(in) :: quintessence
	real(dl), dimension(0:2) :: VofPhi
	real(dl) :: phi
	integer :: n
	
	real(dl), parameter :: units = MPC_in_sec**2 /Tpl**2  !convert to units of 1/Mpc^2

	n = quintessence%potential_type
	
	!call MpiStop('Did you define the field potential? If so, go back to the .f90 file and comment this MpiStop!')
	select case (n)
	case (1)
		field_potential(0) = ... ! Fill here for V(phi) and its derivatives
		field_potential(1) = ...
		field_potential(2) = ...
	case (2)
		field_potential(0) = ...
    ...
    grhode=a2*(0.5d0*phidot**2 + a2*this%field_potential(phi,0))
    tot = this%state%grho_no_de(a) + grhode

    adot=sqrt(tot/3.0d0)
    yprime(1)=phidot/adot !d phi /d a
    yprime(2)= -a2**2*this%field_potential(phi,1)/adot
    ...
    clxq=ay(w_ix)
    vq=ay(w_ix+1)
    dgrhoe= phidot*vq +clxq*a**2*this%field_potential(phi,1)
    dgqe= k*phidot*clxq
    ...
    vq=y(w_ix+1)
    ayprime(w_ix)= vq
    ayprime(w_ix+1) = - 2*adotoa*vq - k*z*phidot - k**2*clxq - a**2*clxq*this%field_potential(phi,2)
    ...
    subroutine TQuintessenceJoao_Init(this, State) ! The goal of this function is to initialize every uninitialized variable of TQuintessenceJoao
    use Powell
    class(TQuintessenceJoao), intent(inout) :: this
    class(TCAMBdata), intent(in), target :: State ! The state of the rest of the system
    real(dl) aend, afrom ! For interpolation
    ...
    real(dl) initial_phi, initial_phidot, a2 ! One of the goals of this function is to determine initial_phi which is a bit more complicated
    ...
    call this%TQuintessence%Init(State) ! This part does some things: 1 - Attributes State to this%state; 2 - sets this%is_cosmological_constant to .false.;
	! 3 - sets this%num_perturb_equations = 2

    if (this%use_zc) then
        !Find underlying parameters m,f to give specified zc and fde_zc (peak early dark energy fraction); Joao - I don't think I need this bc this is specific for early DE.
        ...
        if (.false.) then ! This if is completely skipped
    ...
    this%dloga = (-this%log_astart)/(this%npoints-1) ! log step
	...
	! Joao - probably going to use something like this
    ...
    !om1= this%GetOmegaFromInitial(astart,initial_phi,initial_phidot, atol) ! Joao - IS THERE A FUNCTION THAT DOES THIS??? OMG!!!
    !
    !print*, State%omega_de, 'first trial:', om1
    !if (abs(om1-State%omega_de) > this%omega_tol)) then
    !    !if not, do binary search in the interval
    !    OK=.false.
    !    initial_phidot = astart*this%phidot_start(initial_phi2) ! This is still zero
    ...
    do i=1, npoints-1
        ...
        fde(ix) = 1/((this%state%grho_no_de(sampled_a(ix)) +  this%frac_lambda0*this%State%grhov*a2**2) &
            /(a2*(0.5d0* phidot_a(ix)**2 + a2*this%field_potential(y(1),0))) + 1)
    ...
    allocate(this%phi_a(tot_points),this%phidot_a(tot_points)) ! Allocates the field arrays
    allocate(this%ddphi_a(tot_points),this%ddphidot_a(tot_points)) ! Second derivatives
    allocate(this%sampled_a(tot_points), this%fde(tot_points), this%ddfde(tot_points))
    this%sampled_a(1:ix) = sampled_a(1:ix) ! Assigning the "log" half of the evolution
    ...
    do i=1, this%npoints_linear
        ...
        this%fde(ix) = 1/((this%state%grho_no_de(aend) +  this%frac_lambda0*this%State%grhov*a2**2) &
            /(a2*(0.5d0* this%phidot_a(ix)**2 + a2*this%field_potential(y(1),0))) + 1)
        if (max_ix==0 .and. this%fde(ix)< this%fde(ix-1)) then
            max_ix = ix-1
        end if 
    end do
	
	! The spline subroutine is written in subroutines.f90. Its syntax is spline (x(in), y(in), n(in), d11(in), d1n(in), d2(out)) where:
	! x and y are n-sized arrays for "sample" points (x,y); d11, d1n are the derivatives at the extreme points and d2 is the array of second derivatives.
    call spline(this%sampled_a,this%phi_a,tot_points,splZero,splZero,this%ddphi_a) ! Assigning the array of second derivatives so interpolation is possible
    ...
    end subroutine TQuintessenceJoao_Init
    ...
    do i=1, npoints-1
        ...
        call EvolveBackgroundLog(this,NumEqs,aend,y,w(:,1))
        fde(ix) = 1/((this%state%grho_no_de(sampled_a(ix)) +  this%frac_lambda0*this%State%grhov*a2**2) &
            /((0.5d0*y(2)**2/a2 + a2**2*this%field_potential(y(1),0))) + 1)
        if (max_ix==0 .and. ix > 2 .and. fde(ix)< fde(ix-1)) then
            max_ix = ix-1
        end if
    ...
    !real(dl) function GetOmegaFromInitial(this, astart,phi,phidot,atol) ! THISSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS should help me a LOT
    ...
    !
    !GetOmegaFromInitial=(0.5d0*y(2)**2 + field_potential(y(1),0))/this%State%grhocrit !(3*adot**2)
    !
