    ...
    !15/03/2021 - continuing my quintessence module. I figured that I should do my implementations first and then refactor some things such as VofPhi and the module name,
    ! but the priority right now is to understand and implement my own Init function. The function GetOmegaFromInitial can help me!
    ! The idea is that I don't need to mess with the evolve functions since they are already implemented! I only need to initialize which can be some trouble.
	
	! Log - Joao
	! 15/03/2021 - I am now trying to put the code to work without refactoring any names or functionalities after. To do: look for potentials to fill the function;
	! Try to put the code to work just by assigning some value to initial_phi (in the Init function). I might need to delete that part with z_c and fde_zc, but they won't be
	! necessary. Runnable is what I want for now.
    ...
    type, extends(TQuintessence) :: TEarlyQuintessence ! Joao - Will build upon this class
	integer :: potential_type = 1 ! Could make different classes but I think this works better; I set the default as 1.
	real(dl) :: potentialparams(5) ! My potential parameters
        real(dl) :: n = 3._dl ! n,f and m are the potential parameters for the axion model above
	...
	! Fill for potential here. Don't worry because ReadParams is called as soon as memory is allocated so the potential type and params will be determined
	! Look for potentials in the literature!!!
    potential_t = this%potential_type
    select case (potential_t)
    case (1)
	select case (deriv)
	case(0)
	    Vofphi =
    ...
    ! Critical function to understand
...
!
    ...
    subroutine TEarlyQuintessence_ReadParams(quintessence, Ini) ! Gets the potential form and parameters from the .ini file
    use IniObjects
    type(TEarlyQuintessence), intent(inout) :: quintessence
